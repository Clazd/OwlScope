...
export function PageHeader({ title, subtitle, action, className }: PageHeaderProps) {
  return (
    <header
      className={cn("border-b border-rule px-6 py-6", className)}
      style={{
        background: "linear-gradient(180deg, rgba(30,34,40,0.5) 0%, transparent 100%)",
      }}
    >
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          {subtitle && (
            <p data-mono className="type-data text-ink-3 mt-2">
              {subtitle}
            </p>
          )}
        </div>
